...
Window {
    ...
    width: label.implicitWidth + BorealUI.Units.largeSpacing * 1.5
    height: label.implicitHeight + BorealUI.Units.largeSpacing * 1.5
    ...
    Rectangle {
        ...
        radius: windowHelper.compositing ? BorealUI.Theme.mediumRadius : 0
    ...
    function adjustCorrectLocation() {
        ...
        if (posX <= Screen.virtualX)
            posX = Screen.virtualX + BorealUI.Units.smallSpacing

        // top
        if (posY <= Screen.virtualY)
            posY = Screen.virtualY + BorealUI.Units.smallSpacing
