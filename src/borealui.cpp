...
void BorealUI::initializeEngine(QQmlEngine *engine, const char *uri)
{
    Q_ASSERT(QLatin1String(uri) == QLatin1String("BorealUI"));

    int fontId = QFontDatabase::addApplicationFont(":/borealui/kit/images/fonticons/FluentSystemIcons-Regular.ttf");
...
void BorealUI::registerTypes(const char *uri)
{
    Q_ASSERT(QLatin1String(uri) == QLatin1String("BorealUI"));

    qmlRegisterSingletonType<ThemeManager>("BorealUI.Core", 1, 0, "ThemeManager", [](QQmlEngine *engine, QJSEngine *scriptEngine) -> QObject * {
        Q_UNUSED(engine)
...
QUrl BorealUI::componentUrl(const QString &fileName) const
{
    return QUrl(QStringLiteral("qrc:/borealui/kit/") + fileName);
}
